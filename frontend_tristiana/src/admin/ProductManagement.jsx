...
const ProductManagement = () => {
	...
	const fetchProducts = async () => {
		const response = await axios.get(`${API_BASE_URL}/api/admin/products/allProduct?page=${currentPage}&limit=${productsPerPage}`,{
	...
	const handleSubmit = async (e) => {
		...
		formData.append('clothType', newProduct.clothType);
		formData.append('category', newProduct.category);
		formData.append('discount', newProduct.discount);
		formData.append('isLimited', newProduct.isLimited);
		formData.append('limitedQuantity', newProduct.limitedQuantity);
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div>
								...
								<select value={newProduct.category} onChange={(e) => setNewProduct({ ...newProduct, category: e.target.value })} className="w-full p-2 border rounded-lg" required>
							...
							<div>
								...
								<input type="text" value={newProduct.clothType} onChange={(e) => setNewProduct({ ...newProduct, clothType: e.target.value })} className="w-full p-2 border rounded-lg" required />
							</div>
								...
								<div className="grid grid-cols-2 gap-2">
									{['XS', 'S', 'M', 'L', 'XL', 'XXL'].map(size => (
										<div key={size}>
											...
											<input type="number" value={newProduct.stock[size]} onChange={(e) => handleStockChange(e, size)} className="w-full p-2 border rounded-lg" required />
										</div>
									))}
								</div>
							</div>
						...
						<div>
							...
							<input type="text" placeholder="Enter discount (e.g., 10% or 100)" value={newProduct.discount} onChange={(e) => setNewProduct({ ...newProduct, discount: e.target.value })} className="w-full p-2 border rounded-lg" />
						</div>

						<div>
							<label className="block text-sm font-medium mb-1">Description</label>
							<textarea value={newProduct.description} onChange={(e) => setNewProduct({ ...newProduct, description: e.target.value })} className="w-full p-2 border rounded-lg" rows={4} required />
						</div>
							...
							<div className="space-y-2">
								{[0, 1, 2].map((index) => (
									<div key={index}>
										...
										<input type="file" onChange={(e) => handleImageChange(e, index)} className="w-full p-2 border rounded-lg" accept="image/*" />
									</div>
								))}
							</div>
						</div>
				...
				<div className="bg-white rounded-lg shadow-md">
					<div className="p-6">
											...
											<td className="p-2">
												<div className="flex gap-2">
													<button onClick={() => {
														...
														setNewProduct({ ...product, images: [] }); // reset images
						...
						<div className="mt-4 flex justify-end gap-2">
							<button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 bg-gray-200 rounded-lg disabled:opacity-50">
								Previous
							</button>
							<span className="px-4 py-2">Page {currentPage}</span>
							<button onClick={() => setCurrentPage(prev => prev + 1)} disabled={products.length < productsPerPage} className="px-4 py-2 bg-gray-200 rounded-lg disabled:opacity-50">
								Next
							</button>
						</div>
					</div>
